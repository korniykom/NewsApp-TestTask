...
import com.korniykom.newsapp.data.remote.TopHeadlinesRemoteMediator
...
class NewsRepositoryImpl(
    ...
    override fun getTopHeadlines(category: NewsCategory): Flow<PagingData<Article>> {
        val cacheKey = category.apiValue ?: "all"
        val pagingSourceFactory = { database.articleDao().pagingSource(cacheKey) }

        return Pager(
            config = PagingConfig(pageSize = 20, prefetchDistance = 5),
            remoteMediator = TopHeadlinesRemoteMediator(category, httpClient, database),
            pagingSourceFactory = pagingSourceFactory
